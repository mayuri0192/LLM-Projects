...
class GPT3Summarizer:
    def __init__(self, api_key, deepgram_api_key, model_engine = "gpt-3.5-turbo", max_tokens = 4096):
        load_dotenv()
    ...
    def extract_transcript(self, deepgram_api_key, filename, video_url):
        video_url = YouTube(video_url)
        stream = video_url.streams.filter(only_audio=True).first()
        stream.download(filename=f"{video_url.title}.mp3")
    ...
    def summarize(self, file_id, video_url, filename,transcript, max_sentences):
        
        print(f'🤖 Initializing GPT-3 summarizer...')
        print(f'↪ Using model: {self.model_engine}')
        
        #extract and save transcript
        self.extract_transcript(self.deepgram_api_key, filename, video_url)
